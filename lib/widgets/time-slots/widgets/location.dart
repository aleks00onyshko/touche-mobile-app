...
class LocationSelect extends StatelessWidget {
  ...
  const LocationSelect(
      {super.key, required this.locations, required this.onLocationSelected});
  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      ...
      child: DropdownMenu<Location>(
        ...
        dropdownMenuEntries:
            locations.map<DropdownMenuEntry<Location>>((Location location) {
          return DropdownMenuEntry<Location>(
              value: location,
              label: location.displayNames['uk'] ?? 'Location');
        }).toList(),
      ),
    );
  }
